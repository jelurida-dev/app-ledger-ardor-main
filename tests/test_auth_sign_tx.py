...
def test_send_money_tx(backend, navigator, firmware):   
    ...
    if firmware.device == 'stax':
        instructions = [NavInsID.USE_CASE_REVIEW_TAP] * 3 + [NavInsID.USE_CASE_REVIEW_CONFIRM]
    else:
...
def test_ardor_coin_exchange(backend, navigator, firmware):
    ...
    if firmware.device == 'stax':
        instructions = [NavInsID.USE_CASE_REVIEW_TAP] * 2 + [NavInsID.USE_CASE_REVIEW_CONFIRM]
    else:
