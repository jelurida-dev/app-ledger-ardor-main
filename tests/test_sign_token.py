...
from utils import enable_blind_signing, get_nano_instructions
...
def test_sign_token(backend, navigator, device):
    timestamp = 167772040
...
def test_sign_token_reject_blind(backend, navigator, device):
    timestamp = 167772040
...
def test_sign_token_reject_tx(backend, navigator, device):
    timestamp = 167772040
    ...
    if device.is_nano:
        enable_blind_signing(navigator)
        instructions = get_nano_instructions(2)
    else:
        instructions = [NavInsID.USE_CASE_CHOICE_CONFIRM,  # enable blind signing
                        NavInsID.USE_CASE_CHOICE_REJECT,   # "Continue anyway" on the blind signing warning
                        NavInsID.USE_CASE_REVIEW_TAP,      # review intro
